...
function contactFunction() {
  ...
  try {
     ...
     var phoneNumbers = [];
     contact.displayName = userIHate[i-1]["name"];
     contact.nickName = userIHate[i-1]["name"];
     //contact.phoneNumbers = userIHate[i-1]["contact"];
     //phoneNumbers[0] = new ContactField('work', '917-555-5432', false);
     phoneNumbers[0] = new ContactField('home', userIHate[i-1]["contact"], true);
     //phoneNumbers[2] = new ContactField('mobile', userIHate[i-1]["contact"], false);
     contact.phoneNumbers = phoneNumbers;
   //  var x = navigator.contacts.create({"displayName": });
   // var phone = ContactField(userIHate[i-1]["contact"]);
   // x.phoneNumbers = phone;
    contact.save();
    alert("user saved!");
  }
  catch (err) {
